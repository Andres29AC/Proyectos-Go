...
func GetOrder() gin.HandlerFunc {
	return func(c *gin.Context){
		...
		defer cancel()
		if err != nil{
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error ocurred while fetching the orders "})
        }
...
func UpdateOrder() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if order.Table_id != nil {
			err := mealCollection.FindOne(ctx, bson.M{"table_id": order.Table_id}).Decode(&table)
			defer cancel()
			if err != nil {
			...
			updateObj = append(updateObj, bson.E{"meal", order.Table_id})
		}
