...
func GetInvoice() gin.HandlerFunc {
   return func(c *gin.Context) {
		...
		invoiceView.Payment_due_date = invoice.Pay_due_date

		invoiceView.Payment_method = "null"
		if invoice.Pay_method != nil {
			invoiceView.Payment_method = *invoice.Pay_method
		}

		invoiceView.Invoice_id = invoice.Invoice_id
		invoiceView.Payment_status = *&invoice.Pay_status
		invoiceView.Payment_due = allOrderItems[0]["pay_due_date"]
...
func CreateInvoice() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		status := "PENDING"
		if invoice.Pay_status == nil {
			invoice.Pay_status = &status
		}

		invoice.Pay_due_date, _ = time.Parse(time.RFC3339, time.Now().AddDate(0, 0, 1).Format(time.RFC3339))
...
func UpdateInvoice() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if invoice.Pay_method != nil {
			updateObj = append(updateObj, bson.E{"pay_method", invoice.Pay_method})
		}

		if invoice.Pay_status != nil {
			updateObj = append(updateObj, bson.E{"pay_status", invoice.Pay_status})
		}
